...
from app.models import Login, AdminNotification, Trip, TripImage, Booking, TripUpdate, TripChat, Review, Report, TravelerNotification
from django.db.models import Q, Sum, Count
from app.utils import send_verification_otp
from django.http import JsonResponse, HttpResponse
from django.conf import settings
from django.http import FileResponse
from app.user_details_function import admin_get_user_details
...
def index(request):
    featured_trips = Trip.objects.filter(status='APPROVED').order_by('-created_at')[:3]
...
def company_register_view(request):
    ...
    if not SystemSetting.get_settings().allow_registrations:
        messages.error(request, "New partner registrations are temporarily disabled by the administrator. Please contact support.")
        return redirect('index')

    if request.method == 'POST':
        # Company Info
        company_name = request.POST.get('companyName', '').strip()
        reg_number = request.POST.get('registrationNumber', '').strip()
        email = request.POST.get('email', '').strip()
        phone = request.POST.get('phone', '').strip()
        
        # New Fields
        username = request.POST.get('username', '').strip()
        contact_first_name = request.POST.get('contactFirstName', '').strip()
        contact_last_name = request.POST.get('contactLastName', '').strip()
        ...
        if success:
            messages.success(request, "Company registration successful! A verification code has been sent to your email.")
        else:
            messages.info(request, "Company registration successful! Please click 'Resend Code' on the next page to receive your verification email.")
...
def edit_company_profile(request):
    ...
    if request.method == 'POST':
        ...
        user.razorpay_key_id = request.POST.get('razorpay_key_id')
        user.razorpay_key_secret = request.POST.get('razorpay_key_secret')
...
@login_required
def export_traveler_manifest(request, trip_id):
    """Export the travelers list for a specific trip"""
    if request.user.usertype != 'company' and not request.user.is_superuser:
        return redirect('index')
        
    trip = get_object_or_404(Trip, id=trip_id)
    if trip.company != request.user and not request.user.is_superuser:
        return redirect('index')
        
    response = HttpResponse(content_type='text/csv')
    response['Content-Disposition'] = f'attachment; filename="manifest_{trip.title.replace(" ", "_")}.csv"'
    
    writer = csv.writer(response)
    writer.writerow(['Booking ID', 'Traveler Name', 'Email', 'Gender', 'Age', 'Total Cost', 'Booking Date'])
    ...
    for booking in bookings:
        ...
        if traveler.birth_date:
            age = (date.today() - traveler.birth_date).days // 365
            
        name = f"{traveler.first_name} {traveler.last_name}".strip() or traveler.username
        writer.writerow([
            f"#{booking.id}",
            name,
            traveler.email,
            traveler.gender or 'N/A',
            age,
            booking.total_price,
...
def admin_settings(request):
    ...
    notifications = AdminNotification.objects.filter(is_read=False).order_by('-created_at')
    return render(request, 'admin/admin_settings.html', {'verification_notifications': notifications})
...
def company_reviews(request):
    """View for companies to see reviews received for their trips"""
    if getattr(request.user, 'usertype', None) != 'company':
    ...
    # Get all reviews for trips owned by this company
    reviews = Review.objects.filter(trip__company=request.user).select_related('trip', 'reviewer').order_by('-created_at')
    
    # Calculate stats
    total_reviews = reviews.count()
    if total_reviews > 0:
        avg_overall = reviews.aggregate(Avg('rating_overall'))['rating_overall__avg'] or 0
        avg_vibe = reviews.aggregate(Avg('rating_social_vibe'))['rating_social_vibe__avg'] or 0
        avg_logistics = reviews.aggregate(Avg('rating_logistics'))['rating_logistics__avg'] or 0
        avg_safety = reviews.aggregate(Avg('rating_safety'))['rating_safety__avg'] or 0
    else:
        avg_overall = avg_vibe = avg_logistics = avg_safety = 0
